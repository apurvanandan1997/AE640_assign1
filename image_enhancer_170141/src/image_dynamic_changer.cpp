...
#include <image_enhancer_170141/ImageChangerConfig.h>

int brightness_change=0;
double contrast_factor=1.0;
image_transport::Publisher pub;

void callback(image_enhancer_170141::ImageChangerConfig &config, uint32_t level)
{
        brightness_change=config.brightness_change;
...
int main(int argc, char **argv)
{
    ...
    dynamic_reconfigure::Server<image_enhancer_170141::ImageChangerConfig> server;
    dynamic_reconfigure::Server<image_enhancer_170141::ImageChangerConfig>::CallbackType f;
    f = boost::bind(&callback, _1, _2);
    server.setCallback(f);

    pub = it.advertise("dynamic_cont_changer", 1);
